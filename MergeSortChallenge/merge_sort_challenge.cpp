...
void parallel_merge_sort(int * array, unsigned int left, unsigned int right, unsigned int depth=0) {
    ...
    if (USE_OMP)
    {
        if (left < right) {
                ...
                //https://www.ibm.com/docs/en/zos/3.1.0?topic=programs-using-openmp-directives 
    ...
    else
    {
        ...
        if (depth >= std::log(std::thread::hardware_concurrency())) {
            ...
            unsigned int mid = (left + right) / 2; // find the middle point
            std::thread thread_left(parallel_merge_sort, array, left, mid, depth+1); // sort the left half
...
int main() {
	...
	const int N = 10000; // number of elements to sort
