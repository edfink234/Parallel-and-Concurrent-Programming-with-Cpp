...
size_t parallel_image_downloader(int num_images) 
{
     ...
     * YOUR CODE GOES HERE *
     ***********************/
    size_t num_threads = ((num_images >= std::thread::hardware_concurrency()) ? std::thread::hardware_concurrency() : num_images);
    boost::asio::thread_pool pool(num_threads);
    for (int i=1; i<=num_images; i++) 
    {
        boost::asio::post(pool, [i, &total_bytes]()
        {
...
size_t download_image(int image_num) { 
	...
	snprintf(url, sizeof(url)/sizeof(url[0]), "http://699340.youcanlearnit.net/image%03d.jpg", ((image_num % 50) + 1)); //image url to download
